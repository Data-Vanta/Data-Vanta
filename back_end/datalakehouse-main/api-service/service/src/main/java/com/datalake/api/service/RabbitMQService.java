...
public class RabbitMQService {
    ...
    /**
     * Send query job to RabbitMQ queue as a JSON payload.
     */
    public void sendQueryJob(QueryJob job) {
        try {
            // Convert job to JSON
            String message = objectMapper.writeValueAsString(job);

            log.info("Sending query job to RabbitMQ - JobId: {}, Queue: {}", job.getJobId(), queueName);

            // Send message to queue
            rabbitTemplate.convertAndSend(queueName, message);

            log.info("Query job successfully sent to RabbitMQ: {}", job.getJobId());

        } catch (Exception e) {
            log.error("Failed to send query job to RabbitMQ - JobId: {}", job.getJobId(), e);
            throw new RuntimeException("Failed to queue query job: " + e.getMessage(), e);
        }
    }
}