...
class AuthController {
     ...
     * Used by /verify-email page when a 403 bounce tells the user their
     * account isn't verified yet.
     */
    resendVerification = asyncFun(async (req, res) => {
        const result = await authService.resendVerification(req.user);
        if (result.alreadyVerified) {
            return response.success(res, "Already verified", { alreadyVerified: true });
        }
        return response.success(res, "Verification email resent");
    });
}
