...
/**
 * Same as validateAuth but does NOT require the user to be verified.
 * Used for endpoints that specifically help unverified users (e.g. resend
 * verification email).
 */
exports.validateAuthAllowUnverified = asyncFun(async (req, res, next) => {
    const token = req.headers['x-auth-token'];
    if (!token) return response.fail(res, "Access denied. No token provided.", [], 401);
